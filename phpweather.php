 ...
 * @see	     $decoded_metar
 */
class phpweather5 extends data_retrieval5 {
 ...
 /**
   ...
   */
	function decode_metar($index=false) {
		...
		$this->debug('tmp_metar='.$tmp_metar);
		...
		if ($index===false) { /* We are getting the current METAR */
			...
			if ($this->metar_arch===false) { /* error */
				$this->debug('ERROR: metar_arch===false for:'.$tmp_metar);
				return false;
			} 
		...
		$this->debug('parts='.print_r($parts,true));
		...
		for ($i = 0; $i < $num_parts; $i++) {
			...
			# if (strstr($part,'RMK|TEMPO|BECMG|INTER')) {
			if (preg_match('/(RMK|TEMPO|BECMG|INTER)/',$part, $regs)) {
				$this->debug('+++ Found remark ='.print_r($part,true));
			...
			// -----------------------------------------------------------------------------------
				...
				$this->debug('+++ Found METAR='.print_r($part,true));
				/*
			...
			// -----------------------------------------------------------------------------------
				...
				$this->debug('+++ Found SPECIAL ='.print_r($part,true));
				/*
				 * Type of Report: SPECI
				 */
				$decoded_metar['type'] = 'SPECI';
			// -----------------------------------------------------------------------------------
			} elseif (preg_match('/^[A-Z]{4}$/', $part)/*ereg('^[A-Z]{4}$', $part)*/ && empty($decoded_metar['icao']))  {
				$this->debug('+++ Found Station Identifier ='.print_r($part,true));
				/*
				 * Station Identifier
				 */
				$decoded_metar['icao']  = $part;
				$this->debug('decoded_metar["icao"]:'.$decoded_metar['icao']);

			// -----------------------------------------------------------------------------------
			} elseif (preg_match('/(AUTO|COR|RTD|CC[A-Z]|RR[A-Z])/', $part, $regs)/*ereg('(AUTO|COR|RTD|CC[A-Z]|RR[A-Z])', $part, $regs)*/) {
				$this->debug('+++ Found Report Modifier: AUTO, COR, CCx or RRx ='.print_r($part,true));
				/*
				 * Report Modifier: AUTO, COR, CCx or RRx
				 */
				$decoded_metar['report_mod'] = $regs[1];
				$this->debug('report_mod:'.$decoded_metar['report_mod']);
			// -----------------------------------------------------------------------------------
			} elseif (preg_match('/([0-9]{3}|VRB)([0-9]{2,3})G?([0-9]{2,3})?(KT|MPS|KMH)/', $part, $regs)
						/*ereg('([0-9]{3}|VRB)([0-9]{2,3})G?([0-9]{2,3})?(KT|MPS|KMH)', $part, $regs)*/) {
				$this->debug('+++ Found Wind Group ='.print_r($part,true));
				/* Wind Group */		
			...
			// -----------------------------------------------------------------------------------
				...
				$this->debug('+++ Found Variable wind-direction ='.print_r($part,true));
				/*
			...
			// -----------------------------------------------------------------------------------
				...
				/*ereg('^([0-9]{4})([NS]?[EW]?)$', $part, $regs)*/) {
				$this->debug('+++ Found Visibility in meters (4 digits only) ='.print_r($part,true));	
				// This may repeat if more than one
				/* 
				...
				$this->debug('+++ Found Wind Group and decodes as ='.print_r($group,true));
				
			// -----------------------------------------------------------------------------------
			} elseif (preg_match('/^[0-9]$/', $part) /*ereg('^[0-9]$', $part)*/) {
				$this->debug('+++ Found Temp Visibility Group, single digit followed by space. ='.print_r($part,true));	
				/*
			...
			//} elseif (ereg('^M?(([0-9]?)[ ]?([0-9])(/?)([0-9]*))SM$', $temp_visibility_miles . ' ' . $part, $regs)) {
				...
				$this->debug('+++ Found Visibility Group ='.print_r($part,true));	
				/*
				 * Visibility Group
				 */
				unset($group);
				...
				$decoded_metar['visibility'][] = $group;
				$this->debug('Final Visibility Group ='.print_r($group,true));	
			// -----------------------------------------------------------------------------------
			} elseif ($part == 'CAVOK') {
				$this->debug('+++ Found CAVOK Group ='.print_r($part,true));	
			...
			//} elseif (ereg('^R([0-9]{2})([RLC]?)/([MP]?)([0-9]{4})' .
				...
				/* Runway-group */
				$this->debug('+++ Found Runway-group Group ='.print_r($part,true));	
				unset($group);
				$group['nr'] = $regs[1];
				...
				$this->debug('Final Runway-group ='.print_r($group,true));	
			...
			//} elseif (ereg('^(VC)?' .                           /* Proximity */
				 ...
				 '(PO|SQ|FC|SS)?$/',                   /* Other */
				 $part, $regs)) {
				$this->debug('+++ Found Current weather-group ='.print_r($part,true));	
				/*
			...
			// -----------------------------------------------------------------------------------	
				...
				/* Cloud-group */
				$this->debug('+++ Found Cloud-group ='.print_r($part,true));	
			...
			//} elseif (ereg('^(VV|FEW|SCT|BKN|OVC)([0-9]{3}|///)' .
				...
				$this->debug('+++ We have found (another) a cloud-layer-group.='.print_r($part,true));	
				unset($group);
			...
			// -----------------------------------------------------------------------------------
				...
				$this->debug('+++ We have found Temperature/Dew Point Group.='.print_r($part,true));	
				/*
				...
				if (!empty($regs[2]) && $regs[2] != '//') {
					$this->debug('+++ We have found UK use // instead of the missing temperature... Group='.print_r($part,true));	
			...
			//} elseif (ereg('A([0-9]{4})', $part, $regs)) {
				...
				$this->debug('+++ We have found The pressure measured in inHg.='.print_r($part,true));	
				/*
				 * Altimeter.
			...
			//} elseif (ereg('Q([0-9]{4})', $part, $regs)) {
				 ...
				 */
				$this->debug('+++ We have found Pressure Group.='.print_r($part,true));	
			...
			//} elseif (ereg('^T([0-9]{4})([0-9]{4})', $part, $regs)) {
				...
				$this->debug('+++ We have found Temperature/Dew Point Group, coded to tenth of degree Celsius ='.print_r($part,true));	
				/*
				 * Temperature/Dew Point Group, coded to tenth of degree Celsius.
				 */
				$this->store_temp($regs[1] / 10, $decoded_metar['temperature']['temp_c'], $decoded_metar['temperature']['temp_f']);
				$this->store_temp($regs[2] / 10, $decoded_metar['temperature']['dew_c'], $decoded_metar['temperature']['dew_f']);
			// -----------------------------------------------------------------------------------
			//} elseif (ereg('^T([0-9]{4}$)', $part, $regs)) {
			} elseif (preg_match('/^T([0-9]{4}$)/', $part, $regs)) {
				$this->debug('+++ We have found alternative Temperature/Dew Point Group, coded to tenth of degree Celsius ='.print_r($part,true));	
				$this->store_temp($regs[1], $decoded_metar['temperature']['temp_c'], $decoded_metar['temperature']['temp_f']);
			// -----------------------------------------------------------------------------------
			//} elseif (ereg('^1([0-9]{4}$)', $part, $regs)) {
			} elseif (preg_match('/^1([0-9]{4}$)/', $part, $regs)) {
				$this->debug('+++ We have found 6 hour maximum temperature Celsius, coded to tenth of degree='.print_r($part,true));	
				/*
				 * 6 hour maximum temperature Celsius, coded to tenth of degree
				 */
				$this->store_temp($regs[1] / 10, $decoded_metar['temp_min_max']['max6h_c'], $decoded_metar['temp_min_max']['max6h_f']);
			// -----------------------------------------------------------------------------------
			//} elseif (ereg('^2([0-9]{4}$)', $part, $regs)) {
				 ...
				 */
				$this->store_temp($regs[1] / 10, $decoded_metar['temp_min_max']['min6h_c'], $decoded_metar['temp_min_max']['min6h_f']);
			// -----------------------------------------------------------------------------------
			//} elseif (ereg('^4([0-9]{4})([0-9]{4})$', $part, $regs)) {
				 ...
				 * tenth of degree
				 */
				$this->store_temp($regs[1] / 10, $decoded_metar['temp_min_max']['max24h_c'], $decoded_metar['temp_min_max']['max24h_f']);
				$this->store_temp($regs[2] / 10, $decoded_metar['temp_min_max']['min24h_c'], $decoded_metar['temp_min_max']['min24h_f']);
			// -----------------------------------------------------------------------------------
			//} elseif (ereg('^P([0-9]{4})', $part, $regs)) {
			} elseif (preg_match('/^P([0-9]{4})/', $part, $regs)) {
				$this->debug('+++ We have found Precipitation during last hour in hundredths of an inch='.print_r($part,true));	
				/*
			...
			//} elseif (ereg('^6([0-9]{4})', $part, $regs)) {
				...
				$this->debug('+++ We have found Precipitation during last 3 or 6 hours in hundredths of an inch='.print_r($part,true));	
				/*
			...
			//} elseif (ereg('^7([0-9]{4})', $part, $regs)) {
				...
				$this->debug('+++ We have found Precipitation during last 24 hours in hundredths of an inch.='.print_r($part,true));	
				/*
			...
			//} elseif (ereg('^4/([0-9]{3})', $part, $regs)) {
				...
				$this->debug('+++ We have found Snow depth in inches='.print_r($part,true));	
				/*
				...
				$this->debug("Warning: Unable to match the group '$part', we assume that it was a remark");	
				/*
		...
		// by the time we drop out here we should have parsed the entire text/array
		$this->debug('Parsing of input completed');	
		// -----------------------------------------------------------------------------------
		/*
		 * Relative humidity
		 */
		$fImpossible = -9999.99; // set everything to impossibly low
		$temp_c = $fImpossible;
		$dew_c = $fImpossible;
		$temp_f = $fImpossible;
		$dew_f = $fImpossible;
		...
		$temp_c =$decoded_metar['temperature']['temp_c'];
		$dew_c = $decoded_metar['temperature']['dew_c'];
		$temp_f = $decoded_metar['temperature']['temp_f'];
		$dew_f = $decoded_metar['temperature']['dew_f'];
		
		if ( count($decoded_metar['temperature']) == 4){ // should have two temps in C and F
			$rh_f = number_format(pow(10, (1779.75 * ($dew_c - $temp_c) / ((237.3 + $dew_c) * (237.3 + $temp_c)) + 2)), 1);
			$decoded_metar['rel_humidity'] = $rh_f;
			$this->debug("+++ Compute relative humidity ...".$decoded_metar['rel_humidity']."% TempC=".$decoded_metar['temperature']['temp_c']."Temp DewC=".$decoded_metar['temperature']['dew_c']);	
		} else {
			// no relative humidity
			$this->debug('--- Warning: We have not found relative humidity ...TempC='.$decoded_metar['temperature']['temp_c']."Temp DewC=".$decoded_metar['temperature']['dew_c']);	
		}
		// -----------------------------------------------------------------------------------
		/*
		 *  Compute windchill if temp < 40f and windspeed > 3 mph
		 */
		$wind_mph = -1; // impossible
		$wind_mph = $decoded_metar['wind']['miles_per_hour'];
		if ( ($temp_f != $fImpossible)&& ($temp_f <= 40 ) && ($wind_mph > 3) )
		{
			$decoded_metar['windchill']['windchill_f'] = number_format(35.74 + 0.6215*$temp_f - 35.75*pow((float)$wind_mph, 0.16) + 0.4275*$temp_f * pow((float)$wind_mph, 0.16));
			$decoded_metar['windchill']['windchill_c'] = number_format(13.112 + 0.6215*$temp_c - 13.37*pow(($wind_mph/1.609), 0.16) + 0.3965*$temp_c * pow(($wind_mph/1.609), 0.16));
			$this->debug("+++ We have computed windchill. temp_f=".$temp_f." and wind_mph=".$wind_mph." WC-C=".$decoded_metar['windchill']['windchill_c']." and WC-F".$decoded_metar['windchill']['windchill_f']);	
		} else {
			// no relative humidity
			$this->debug('--- We have not computed windchill. temp_f='.$temp_f." and wind_mph=".$wind_mph);	
		}
			// -----------------------------------------------------------------------------------
		/*
		 * Compute heat index if temp > 70F
		 */
		if ( ($temp_f > 70) && ($rh_f != $fImpossible) ) { // both must be set
			...
			$this->debug("+++ We have computed the heat index if temp > 70F. He_I C=".$decoded_metar['heatindex']['heatindex_c']." He_I F=".$decoded_metar['heatindex']['heatindex_f']);	
		} else {
			if (($temp_f <= 70)){
				$this->debug("--- We have not computed heat index if temp > 70F as temperature is too cold. temp_f=".$temp_f." RH_f=".$rh_f);	
			} else {
				$this->debug("--- We have not computed heat index if temp > 70F temp_f=".$temp_f." RH_f=".$rh_f);	
			}
		}
		 ...
		 */
		if ($rh_f != $fImpossible) {
			$e = (6.112 * pow(10, 7.5 * $temp_c / (237.7 + $temp_c)) * $decoded_metar['rel_humidity'] / 100) - 10;
			$decoded_metar['humidex']['humidex_c'] = number_format($temp_c + 5/9 * $e, 1);
			$decoded_metar['humidex']['humidex_f'] = number_format($decoded_metar['humidex']['humidex_c'] * 9/5 + 32, 1);
			$this->debug("+++ We have computed the humidity index. Hu_I_C=".$decoded_metar['humidex']['humidex_c']." Hu_I_F=".$decoded_metar['humidex']['humidex_f']);	
		} else {
			$this->debug('--- We have not computed the humidity index');	
		}
		// -----------------------------------------------------------------------------------
