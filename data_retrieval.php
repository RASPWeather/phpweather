 ...
 */
class data_retrieval5 extends db_layer {
  ...
  function get_file_socket($host,$location) {
    ...
    if ($this->properties['use_proxy']) { 
      ...
      $request = "GET https://$host$location $request" .
        "Host: $host\r\n" .
    ...
    } else { echo "Failed to connect for:".$request." From:".$host; exit(-1); }
  ...
  function get_metar_socket($icao) {
    ...
    $location = "/data/observations/metar/stations/$icao.TXT";
...
/* http://tgftp.nws.noaa.gov/data/observations/metar/stations/ */
  ...
  function get_metar_file($icao) {
    $host = 'tgftp.nws.noaa.gov'; /* 'weather.noaa.gov'; */
    $location = "/data/observations/metar/stations/$icao.TXT";
	$result = @file('https://' . $host . $location);
    return @file('https://' . $host . $location);
  }
  ...
  function get_metar_from_web($new_station) {
	...
	$this->debug('metar_data='.print_r($metar_data,true));
    ...
    if (!empty($metar_data)) {
      ...
      //$metar = ereg_replace("[\n\r ]+", ' ', trim(implode(' ', $metar_data)));
		$metar = trim(implode(' ', $metar_data)); // preg_replace("[\n\r ]+", ' ', trim(implode(' ', $metar_data)));
      
		$this->debug('metar='.$metar);
 ...
 /**
  ...
  function get_taf_file($icao) {
    $host = 'tgftp.nws.noaa.gov'; /* 'weather.noaa.gov'; */
    $location = "/data/forecasts/taf/stations/$icao.TXT";
    $this->debug("getting TAF from location: https://$host/$location");
    return @file('https://' . $host . $location);
  }
