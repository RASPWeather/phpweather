...
if (@file('https://tgftp.nws.noaa.gov' . $dir)) {
  success('Fetched METAR from <code>https://tgftp.nws.noaa.gov</code>.');
} else {
  error('Could not fetch METAR from <code>https://tgftp.nws.noaa.gov</code>.');
}
...
if ($fp) {
  if ($use_proxy)
    ...
    success("Connection established to <code>$host:443</code>.");
